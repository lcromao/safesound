...
from src.schemas.transcribe_schemas import (
    TranscriptionRequest, TranscriptionResponse, TranscriptionProgress,
    WhisperModel, TranscriptionAction, ErrorResponse
)
...
async def upload_audio(
    ...
    try:
        ...
        text, detected_language, processing_time = whisper_service.transcribe_audio(
...
async def transcribe_stream(websocket: WebSocket):
    ...
    try:
        ...
        logger.info(f"Starting real-time transcription with model: {model}, action: {action}")
        ...
        while True:
            try:
                ...
                if len(data) > 0:
                    ...
                    # Transcribe chunk
                    text = whisper_service.transcribe_audio_chunk(data, model, action)
